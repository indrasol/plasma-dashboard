...
export default function DonorPanel() {
 ...
 const [loyaltyCounts, setLoyaltyCounts] = useState({});
 ...
 useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error("❌ Error loading donor segments:", err.message || err);
      }
   ...
   <div className="section">
     ...
     {loading ? (
       ...
       <>
         ...
         {/* Embedded Dashboards */}
         <>
            {/* Lookalike Dashboard now fully self-contained */}
            {!loading && (<LookalikeDashboard />)}

            {!loading && (<ClusterSummaryPanel />)}

            {!loading && (<div style={{ marginTop: '40px' }}>
             	<h3>🧬 Cluster Visualization (PCA)</h3>
             	<img src="/assets/cluster_plot.png" alt="PCA Plot" width={'600'} />
           	</div>)}
