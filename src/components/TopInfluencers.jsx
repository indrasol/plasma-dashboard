    ...
    async function loadInterests() {
      try {
        ...
        const categories = [...new Set(
          data.map(row => row.interest_category?.trim()).filter(Boolean)
        )].sort();

        const names = [...new Set(
          data.map(row => row.interest_name?.trim()).filter(Boolean)
        )].sort();
        ...
        console.error("❌ Failed to load interests:", err.message || err);
      } finally {
