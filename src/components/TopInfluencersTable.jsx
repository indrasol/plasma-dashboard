...
export default function TopInfluencersTable({ selectedCategory, selectedInterestName }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!linksRaw?.length || !scoresRaw?.length) {
          setAllRows([]);
          return;
        }
        ...
        const validDonorIdsSet = new Set((filteredDonors || []).map(d => d.donor_id));
        ...
        setAllRows(enrichedRows);
         ...
         console.error("❌ Error loading influencer data:", err.message);
         setAllRows([]);
      } finally {
         setLoading(false);
      }
